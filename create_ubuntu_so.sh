...
# PASO 3: Compilar CADA archivo .py a .so para ARM64
# RUN ON UBUNTU THIS WILL MAKE ARM64 ON UBUNTU 
find src -name "*.py" ! -name "main.py" | while read pyfile; do
    module=$(basename "${pyfile%.py}")
    module_path=$(dirname "$pyfile")
    echo "   ➜ Compilando: $module"
    
    # Generar código C con Cython
    cython -3 --embed "$pyfile" -o "${pyfile%.py}.c"
    
    # Compilar a .so para ARM64
    aarch64-linux-android-clang -shared -pthread \
        -I$PREFIX/include \
        -I$PREFIX/include/python3.11 \
        "${pyfile%.py}.c" \
        -o "${module_path}/${module}.so"
